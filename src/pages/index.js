  ...
  <Layout>
    ...
    <div className="not-prose min-h-screen mt-12">
      <div className="flex flex-col justify-start items-start gap-2 mb-6">
        ...
        <h2 className="lg:text-xl">{data.allMarkdownRemark.totalCount} Posts</h2>
      </div>
      {
        data.allMarkdownRemark.edges.map(({ node }) => (
          <div key={node.fields.slug} className="flex flex-col justify-start items-start gap-2 mb-8 lg:mb-16 overflow-auto">
            <Link to={node.fields.slug} aria-label={node.fields.slug}>
            ...
            <Link to={node.fields.slug} aria-label={node.fields.slug} className="md:text-2xl font-semibold text-mainPurple lg:mt-2">Read more</Link>
          </div>
        ))
      }
    </div>
